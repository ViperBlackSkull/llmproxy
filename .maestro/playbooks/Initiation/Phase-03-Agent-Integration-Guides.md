...
- [x] Discover each agent's real base-URL and auth config surface on this machine, and record findings in a scratch note (`.maestro/playbooks/Initiation/Working/agent-config-discovery.md`):
...
- [x] Write `docs/agents/claude-code.md` — structured doc with YAML front matter (`type: reference`, `tags: [claude-code, setup, credential-free]`, `related: '[[Agent-Setup-Overview]]'`, `created:` today):
...
- [x] Write `docs/agents/codex.md` (same front-matter pattern, `tags: [codex, setup, credential-free]`):
...
- [x] Write `docs/agents/opencode.md` and `docs/agents/pi.md` (same front-matter pattern):
...
- [x] Write `docs/agents/README.md` titled "Agent Setup Overview" (file name `README.md` inside `docs/agents/`, wiki-link target `[[Agent-Setup-Overview]]`):
...
- [x] Cross-check every snippet against the daemon's routing reality and commit:
  ...
  > Done 2026-09-03. Path cross-check: Claude Code/Pi emit `/v1/messages` → anthropic backend (both `.env` vars live); Codex (`/v1/responses` under `wire_api = "responses"`, derived from this machine's own config.toml) and OpenCode (`/v1/chat/completions`) → openai backend, and since live `.env` has no `OPENAI_API_KEY`/`OPENAI_BACKEND`, both guides carry an explicit Prerequisites block naming those two vars (z.ai coding endpoint). Key scan: `grep` for real-key patterns across `docs/agents/` + Working note came back clean; 9 `llmproxy-dummy` usages. README's Credential-Free Mode section now links to `docs/agents/`. Discovery quirk worth remembering: this machine's Claude `settings.json` pins models to `zai/glm-5.3` which matches no default MODEL_MAP rule — the Claude guide documents both fixes (`zai/*=glm-5.3` rule or bare model names).
